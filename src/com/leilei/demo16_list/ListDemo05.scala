...
object ListDemo05 {
  def main(args: Array[String]): Unit = {
    ...
    //zip  压缩（两个list 组合为一个）解压操作
    var nameAgeList = nameList.zip(ageList)

    println(nameAgeList)
    // upZip  解压为了一个tuple2元组
    var unzipResult = nameAgeList.unzip
    println(unzipResult)
    ...
    // 去重
    var distinctList=unionList.distinct
